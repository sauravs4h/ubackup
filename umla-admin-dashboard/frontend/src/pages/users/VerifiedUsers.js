...
const VerifiedUsers = () => {
  ...
  const verifiedUsersData = useSelector((state) => state.getVerifiedUsers);
  const { responseData, loading } = verifiedUsersData;
  console.log(responseData);

  useEffect(() => {
    dispatch(getVerifiedUsers(currentPage, searchInput));
  ...
  return (
    <div>
      <div className="bg-white p-4 flex  justify-between rounded-lg items-center gap-4 ">
        ...
        <Link
          to="/users"
          className="text-white bg-[#515ada] p-2 px-6 rounded-lg"
        >
          Back
        </Link>
      </div>
