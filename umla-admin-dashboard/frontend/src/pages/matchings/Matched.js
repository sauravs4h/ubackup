...
import { getMatched } from "../../redux/actions/partnerActions";
...
const Matched = () => {
  ...
  const matchedData = useSelector((state) => state.getMatched);
  const { matched, loading } = matchedData;
  ...
  useEffect(() => {
    if (matched) {
      setPageCount(Math.ceil(matched.length / 20));
    }
  }, [matched]); // setting the page count
          ...
          <table className="min-w-full ">
            <thead className="border-b-2">
              <tr className="text-[#646464] font-normal text-base">
                ...
                <th className="pb-2 font-normal">Matched Name</th>
            ...
            <tbody>
              {matched?.map((match, index) => (
                <tr key={index} className="border-b-[1px]  text-center">
                  ...
                  <td className="py-[13px]">{match?.date}</td>
                  ...
                  <td>{match?.offer}</td>
                  <td style={{ width: "20%" }}>{match?.address}</td>
                  <td>{match?.city}</td>
                  <td>{match?.purchaseOf}</td>
                </tr>
              ))}
            </tbody>
          </table>
